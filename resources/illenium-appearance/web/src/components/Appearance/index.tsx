...
import Options, { AnimationKey } from './Options';
import Modal from '../Modal';
import Tattoos from './Tattoos';

import { Wrapper, LeftPanel, Brand, TabBar, Tab, Container } from './styles';

type MainTab = 'ped' | 'caracteristicas' | 'visual' | 'roupas' | 'acessorios';
...
const Appearance = () => {
  ...
  const [activeTab, setActiveTab] = useState<MainTab>('ped');
  const [animation, setAnimation] = useState<AnimationKey | null>('static');
  ...
  const handlePlayAnimation = useCallback((key: AnimationKey) => {
    setAnimation(key);
    Nui.post('appearance_play_animation', key);
  }, []);
  ...
  useEffect(() => {
    Nui.post('appearance_get_locales').then(result =>
      setLocales(typeof result === 'string' ? JSON.parse(result) : result),
    );
  ...
  useEffect(() => {
    if (display.appearance) {
      (async () => {
        // illenium contract: settings and data come from two separate callbacks.
        const [settingsRes, dataRes] = await Promise.all([
          Nui.post('appearance_get_settings'),
          Nui.post('appearance_get_data'),
        ]);

        const settings = settingsRes?.appearanceSettings ?? settingsRes;
        const _config = dataRes?.config;
        const appearanceData = dataRes?.appearanceData ?? dataRes;
          ...
          item && (
            <animated.div key={key} style={style}>
              <Wrapper>
                <LeftPanel>
                  <Brand>
                    <div className="title">Aparência</div>
                    <div className="sub">Personalização de personagem</div>
                  </Brand>
                  <TabBar>
                    {TABS.map(t => (
                  ...
                  <Container>
                    ...
                    {activeTab === 'caracteristicas' && isPedFreemodeModel && (
                      <>
                        {config.headBlend && (
                          <HeadBlend
                    ...
                    {activeTab === 'visual' && isPedFreemodeModel && (
                      <>
                        {config.headOverlays && (
                          <HeadOverlays
                            settings={{
                    ...
                    {activeTab === 'roupas' && config.components && (
                      <Components
                    ...
                    {activeTab === 'acessorios' && config.props && (
                      <Props
