	...
	local primary = lib.addKeybind({
		...
		onPressed = function()
			...
			if closest and closest.currentDistance < 1.2 and (not closest.instance or closest.instance == currentInstance) then
				...
				elseif closest.inv and closest.inv ~= 'license' and closest.inv ~= 'policeevidence' and closest.inv ~= 'drop' then
...
RegisterNUICallback('splitItem', function(data, cb)
    if usingItem or not invOpen or invBusy then return cb(false) end
    local success = lib.callback.await('ox_inventory:splitItem', false, data.slot, data.count)
    cb(success or false)
end)
