...
lib.callback.register('nv_delivery:startJob', function(source)
...
lib.callback.register('nv_delivery:pickupPackage', function(source, pkgId)
    local src = tonumber(source)
...
lib.callback.register('nv_delivery:completeDelivery', function(source, slotIndex)
    local src = tonumber(source)
    local session = activeJobSessions[src]
    if not session then return false end
    ...
    if exports.ox_inventory:RemoveItem(src, item.name, 1, nil, slotIndex) then
        ...
        if session.deliveredCount >= session.spawnedCount then
            ...
            TriggerClientEvent('nv_delivery:cleanJobState', src)
        end
...
RegisterNetEvent('nv_delivery:cancelJob', function(deleteHandItem)
    local src = tonumber(source)
    local session = activeJobSessions[src]
    ...
    -- Se solicitado, remove os itens de entrega
